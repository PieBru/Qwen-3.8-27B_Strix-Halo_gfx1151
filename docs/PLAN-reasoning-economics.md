...
| [PR #27342](https://github.com/ggml-org/llama.cpp/pull/27342) — DFlash2 upstreamed | The spec path our fork already ships, landing in stock (`draft-dflash` + DFlash2 checkpoints; author eval: 1.85× on M5 Pro) | On merge: stock-vs-fork re-bench becomes meaningful (same spec path both sides); watch whether our fork-sync strategy simplifies; our draft GGUF may finally load in stock builds | On merge: run stock-master spec-battery vs fork; check draft-GGUF compatibility (today: "wrong number of tensors") |
